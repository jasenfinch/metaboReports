...
```{r retainedFeatureMeasures,echo=FALSE}
features <- tibble(Feature = colnames(analysisResults@preTreated$Data))
accurateDat <- processingResults %>% 
  accurateData() %>%
  select(Mode,mz,Purity,Centrality)
feat <- features %>%
  mutate(Mode = str_sub(Feature,1,1),mz = str_replace_all(Feature,'[:alpha:]','') %>% as.numeric()) %>%
  left_join(accurateDat )

purity <- feat %>%
  ggplot(aes(x = Purity)) +
  geom_histogram(fill = ptol_pal()(5)[2],colour = 'black') +
  theme_bw() +
  facet_wrap(~Mode) +
  ggtitle('Purity')
centrality <- feat %>%
